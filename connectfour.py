...
def verify_column():
    '''Makes sure the input is a valid column number'''
    column = input("Please type in a column (one number): ")
    try:
       val = int(column)
    except ValueError:
        print("That's not an number!")
        column = verify_column()
    if 0 < int(column) < BOARD_COLUMNS:
        return column
    else:
        print(column + " is not a valid column number.")

def verify_DorP():
    '''makes sure the input is a d or a p'''
    drop_or_pop = input("Please type in 'd' or 'p' (d for drop, p for pop): ")
    if drop_or_pop.lower() == 'd':
        return drop_or_pop.lower()
    elif drop_or_pop.lower() == 'p':
        return drop_or_pop.lower()
    else:
        print(drop_or_pop + " is not a either d or p")
        return verify_DorP()

def check_drop(game_state, column_number): 
    empty_row = _find_bottom_empty_row_in_column(game_state.board, column_number)
    if empty_row == -1:
        print("That column is full. Input a different column.")
        column_number = int(verify_column())-1
        check_drop(game_state, column_number)
    else:
        return drop(game_state, column_number)

#main module code to host a game between two different players
Game = new_game()
print("Welcome to Connect Four!\nEach turn will display what color it is then will ask for user input.")
print("\nThe user will then input a column number and the letter 'd' or 'p'.")
print("If a 'd' is inputted, the piece will be dropped from above.\nIf a 'p' is inputted, it will pop out a player's piece if it is\nat the bottom.")
print("\nThe game follows standard Connect Four rules.")
...
while game_is_running:
    ...
    if game_won == 1:
        print('RED (Player 1) wins!!!!!')
        break
    elif game_won == 2:
        print('YELLOW (Player 2) wins!!!!!')
        break        
    print("\nTurn: " + str(Game.turn))
    column = int(verify_column())-1
    drop_or_pop = verify_DorP()
    full_loop = True
    while full_loop:
        empty_row = _find_bottom_empty_row_in_column(Game.board, column)
        if empty_row == -1 and drop_or_pop != 'p':
            print("That column is full. Input a different column")
            column = int(verify_column())-1
        elif Game.turn != Game.board[column][BOARD_ROWS - 1] and drop_or_pop == 'p':
            print("You cannot pop ths column. You have to pop your own piece.")
            column = int(verify_column())-1
            drop_or_pop = verify_DorP()
        else:
            break
    if drop_or_pop == 'd':
        Game = check_drop(Game, column)
    elif drop_or_pop == 'p':
        Game = pop(Game, column)
